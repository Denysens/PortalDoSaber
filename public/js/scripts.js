function renderizar_livros(livros) {
    ...
    if (livros instanceof Array) {
        livros.forEach(livro => {
            ...
            card.innerHTML = `
                <img src='public/img/default-book.jpg' alt="${livro.titulo} class="livro-img">
                <div class="livro-informacoes">
                    <h3 class="livro-titulo">Título: ${livro.titulo}</h3>
                    <p class="livro-id"><strong>Código:</strong> ${livro.id_livro}</p>
                    <p class="livro-autor"><strong>Autor:</strong> ${livro.autor || 'Desconhecido'}</p>
                    <p class="livro-ano"><strong>Ano de publicação:</strong> ${livro.ano_publicacao || 'Desconhecido'}</p>
        ...
        card.innerHTML = `
            <img src='public/img/default-book.jpg" alt="${livros.titulo} class="livro-img">
            <div class="livro-informacoes">
                <h3 class="livro-titulo">${livros.titulo || 'Desconhecido'}</h3>
                <p class="livro-id"><strong>Código:</strong> ${livros.id_livro}</p>
                <p class="livro-autor"><strong>Autor:</strong> ${livros.autor || 'Desconhecido'}</p>
                <p class="livro-ano"><strong>Ano de publicação:</strong> ${livros.ano_publicacao || 'Desconhecido'}</p>
...
function renderizar_categorias(categorias) {
    ...
    if (categorias.message) {
        select.innerHTML = "<p> '${categorias.message}'</p>";
        return
    }
...
function pesquisar_livro(event) {
    ...
    const titulo = input_titulo.value.trim(); // Remove espaços em branco no início e no fim
...
function formatar_data(data_string) {
    // Retorna null se a data for indefinida ou vazia
    if (!data_string) {
        return null;
    }
...
function cadastrar_livro(event) {
    ...
    fetch(`${base_url}/cadastrar_livros`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ titulo, autor, editora, ano_publicacao, descricao, id_categoria }),
    })
        .then(res => res.json())
        .then(livros => {
            if (livros.message) {
